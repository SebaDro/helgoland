...
@Component({
  selector: 'n52-custom-d3-timeseries-graph',
  templateUrl: './custom-d3-timeseries-graph.component.html',
  styleUrls: ['./custom-d3-timeseries-graph.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CustomD3TimeseriesGraphComponent
  ...
  private leftBufferSum: number;
  private rightBufferSum: number;
  ...
  protected plotGraph(): void {
      ...
      this.bufferSum = 0;
      this.leftBufferSum = 0;
      this.rightBufferSum = 0;
      ...
      yAxisArray.forEach((entry) => {
          if (entry !== null) {
              ...
              entry.offset = this.leftBufferSum;
              // determine axis orientation
              entry.rightPosition =  yAxisCount % 2 === 0;

              let yAxisResult = this.drawYaxis(entry);
              if (this.yScaleBase === null) {
                  this.yScaleBase = yAxisResult.yScale;
                  this.leftBufferSum = yAxisResult.leftBuffer;
                  this.rightBufferSum = yAxisResult.rightBuffer;
              } else {
                  this.leftBufferSum = yAxisResult.leftBuffer;
                  this.rightBufferSum = yAxisResult.rightBuffer;
              }
      ...
      // draw x and y axis
      this.drawXaxis(this.leftBufferSum, this.rightBufferSum);

      // create background as rectangle providing panning
      this.background = this.graph.append('svg:rect')
          .attr('width', this.width - this.leftBufferSum - this.rightBufferSum)
          .attr('height', this.height)
          .attr('id', 'backgroundRect')
          .attr('fill', 'none')
          .attr('stroke', 'none')
          .attr('pointer-events', 'all')
          .attr('transform', 'translate(' + this.leftBufferSum + ', 0)');
  ...
  protected createHoveringNet(inputData): void {
    ...
    //       bottom = this.margin.top + this.background.node().getBBox().height;
      ...
      let left = this.leftBufferSum, // + this.margin.left,
          top = this.margin.top,
          right = this.background.node().getBBox().width + this.leftBufferSum, // + this.margin.left,
          bottom = this.margin.top + this.background.node().getBBox().height;
                  ...
                  if (dist <= 8) {
                      ...
                      if (coords[0] >= 0 && coords[0] <= rectBack.width && coords[1] >= 0 && coords[1] <= rectBack.height) {
                        ...
                        //   if ((this.background.node().getBBox().width + this.bufferSum) / 2 > coords[0]) { onLeftSide = true; }
                          if ((this.background.node().getBBox().width + this.leftBufferSum) / 2 > coords[0]) { onLeftSide = true; }
  ...
  private calcDistanceHovering(dataset, coords: [number, number]): number {
      let mX = coords[0] + this.leftBufferSum,
          ...
          if (formerTimestamp) {
              ...
              g.append('line')
                ...
                //   .attr('x1', 0 + this.bufferSum + leftRight + 'px')
                  .attr('x1', 0 + this.leftBufferSum + leftRight + 'px')
                  .attr('y1', this.height / 2 + 'px')
                //   .attr('x2', 0 + this.bufferSum + (buttonWidth - leftRight) + 'px')
                  .attr('x2', 0 + this.leftBufferSum + (buttonWidth - leftRight) + 'px')
   ...
   */
  private drawXaxis(bufferLeftXrange: number, bufferRightXrange: number): void {
      // range for x axis scale
      this.xScaleBase = d3.scaleTime()
          .domain([new Date(this.xAxisRange.from), new Date(this.xAxisRange.to)])
          .range([bufferLeftXrange, this.width - bufferRightXrange]);
      ...
      if (this.plotOptions.showTimeLabel) {
          this.graph.append('text')
              .attr('x', (this.width + bufferLeftXrange) / 2)
  ...
  private drawYaxis(entry): YScale {
      ...
      let yAxisRightGen = d3.axisRight(yScale).ticks(5);
      let buffer = 0;
      let leftBuffer = 0;
      let rightBuffer = 0;
      ...
      if (!entry.rightPosition) {
          axis = this.graph.append('svg:g')
            .attr('class', 'y axis')
            .call(yAxisGen);
      } else {
          axis = this.graph.append('svg:g')
      ...
      if (showAxis) {
        ...
        //   buffer = (showAxis ? entry.offset + (axisWidth < this.margin.left ? this.margin.left : axisWidth) : 0);
          leftBuffer = (showAxis ? entry.offset + (axisWidth < this.margin.left ? this.margin.left : axisWidth) : 0);
          rightBuffer = (showAxis ? entry.offset + (axisWidth < this.margin.left ? this.margin.left : axisWidth) : 0);
          const axisWidthDiv = (axisWidth < this.margin.left ? this.margin.left : axisWidth);

          if (!entry.first) {
            //   axis.attr('transform', 'translate(' + buffer + ', 0)');
              if(!entry.rightPosition){
                axis.attr('transform', 'translate(' + leftBuffer + ', 0)');
              } else {
                axis.attr('transform', 'translate(' + (this.width - rightBuffer) + ', 0)');
              }
          } else {
        //       buffer = axisWidthDiv - this.margin.left;
        //       axis.attr('transform', 'translate(' + buffer + ', 0)');
              leftBuffer = axisWidthDiv - this.margin.left;
              axis.attr('transform', 'translate(' + leftBuffer + ', 0)');
          }

          if(!entry.rightPosition){
            let textOff = - (this.leftBufferSum);
            if (entry.first) {
                textOff = this.margin.left;
            }
            text.attr('y', 0 - textOff);
          } else {
            let textOff = - (this.rightBufferSum);
            if(!this.rightBufferSum){
                textOff = this.margin.right;
            }
            text.attr('y', this.width - textOff);
          }
      ...
      if (entry.data.length > 0) {
          ...
          if (getYaxisRange !== undefined) {
              ...
              this.graph
                  ...
                  .append('svg:rect')
                  .attr('x', this.leftBufferSum)
                  .attr('y', 0)
                  .attr('width', this.width - this.leftBufferSum)
                  .attr('height', this.height);
  ...
  private getxDomain(start: number, end: number): [number, number] {
      ...
      start += this.leftBufferSum;
      end += this.leftBufferSum;
